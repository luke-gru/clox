...
TEST_FILES = test/test_object.c test/test_nodes.c test/test_compiler.c test/test_vm.c test/test_chunk.c
...
test: build
	...
	${CC} ${CFLAGS} $(TEST_SRCS) test/test_chunk.c ${TEST_FLAGS} -o bin/test_chunk
	./bin/test_object && ./bin/test_nodes && ./bin/test_compiler && ./bin/test_vm && ./bin/test_chunk
